...
import { ref } from 'vue';
import menujson from "../assets/barMenu.json";
import MenuItems from "./MenuItems.vue";

const sigCocktailsItems = menujson.signatureCocktails;
const beerItems = menujson.beer;
const wineItems = menujson.wine;
const mocktailItems = menujson.mocktail;
const softDrinkItems = menujson.softDrinks;
    ...
    <div class="flex flex-col pt-6">
        <span @click="toggleMenu" class="flex justify-between">
            <span class="flex gap-2">
                <IconLucideWine class="text-[20px]"/>
                <span>
        ...
        <div class="flex flex-col gap-4" v-show="isMenuVisible">
            ...
            <span>
                ...
                <ul>
                    <li v-for="item in sigCocktailsItems" :key="item.name">
                        <MenuItems>
            ...
            <span>
                ...
                <ul>
                    <li v-for="item in beerItems" :key="item.name">
                        <MenuItems :showYenSymbol="item.price !== '*ASK'">
                            <template #item-name>{{ item.name }}</template>
                            <template #item-price>{{ item.price }}</template>
                            <template #item-desc>{{ item.description }}</template>
                        </MenuItems>
            ...
            <div>
                ...
                <ul>
                    <li v-for="item in wineItems" :key="item.name">
                        <MenuItems :showYenSymbol="item.price !== '*ASK'">
                            <template #item-name>{{ item.name }}</template>
                            <template #item-price>{{ item.price }}</template>
                            <template #item-desc>{{ item.description }}</template>
            ...
            <span>
                ...
                <ul>
                    <li v-for="item in mocktailItems" :key="item.name">
                        <MenuItems>
                            <template #item-name>{{ item.name }}</template>
                            <template #item-price>{{ item.price }}</template>
                            <template #item-desc>{{ item.description }}</template>
